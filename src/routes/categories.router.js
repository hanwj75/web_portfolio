...
router.post("/portfolios/:portfolioId/categories", jwtMiddleware, async (req, res, next) => {
  try {
...
// router.get("/categories/:portfolioId", jwtMiddleware, async (req, res, next) => {
//   try {
//     const { portfolioId } = req.params;
//     if (!portfolioId) {
//       throw new CustomError("포트폴리오 ID가 필요합니다.", 400);
 ...
 */
router.get("/portfolios/:portfolioId/categories", async (req, res, next) => {
  try {
    const { portfolioId } = req.params;
    if (!portfolioId) {
      throw new CustomError("포트폴리오 ID가 필요합니다.", 400);
...
router.patch(
  "/portfolios/:portfolioId/categories/reorder",
  jwtMiddleware,
  async (req, res, next) => {
    try {
      const { portfolioId } = req.params;
...
router.patch(
  "/portfolios/:portfolioId/categories/:categoryId",
  jwtMiddleware,
  async (req, res, next) => {
    try {
